...
//bool poweron = true;                // Variable to hold the poweron condition. Set to true if not using remote for on/off.
...
void rcodePower()
{
  ...
  powerState = serialin.substring(1);
}
...
//
// ---------------------------- Process recote control IR commands 
...
void loop()
{
 ...
 //check rotary button (source)
  btnState = digitalRead(SW);

  if (btnState == LOW && !isPressing) {      // button switch (rotary) pressed
    //100msec since last button press, interval of button press should be greater than 100ms
  ...
  if(btnState == HIGH && isPressing) {    //button released
    ...
    finalDuration = millis() - pressStartTime;
  ...
  if(IrReceiver.decode()) {   //library updated
    long  irCmd;
    irCmd = IrReceiver.decodedIRData.decodedRawData & 0xFFFF;   //changed 29/03/26 - reduce computation
    if(prevCmd != irCmd) {
      for (int i=0; i<6; i++) {
